...
class OKX:
    def __init__(self):self.h=httpx.AsyncClient(timeout=12,headers={"User-Agent":"MRC-Slow-Staging/4.2"})
...
STATE={
    ...
    "mode":"PAPER_STAGING","version":"slow-staging-v4.2-atomic","release_sha":RELEASE_SHA,"coverage_gap":None,
    ...
    try:
        while True:
                    ...
                    if process:
                        ...
                        if p and p.decision in ("LONG","SHORT") and not slot:
                            if STATE["coverage_gap"]:
                                ...
                                slot=db.open_trade() is not None
                                action="BLOCKED_ATOMIC_SLOT_OR_REENTRY";db.update_decision_action(eng.STRATEGY,s,int(newmark),action)
            ...
            async with LOCK:
                ...
                STATE["position"]=db.open_trade();STATE["equity"]=round(db.equity(START_EQUITY),2);STATE["recent_trades"]=db.recent();refresh_telemetry()
...
app=FastAPI(title="MRC Slow Trend Staging",version="4.2",lifespan=life)
...
async def health():
    ...
    return JSONResponse({"ok":ok,"feed_ready":bool(ck and all(ck.values())),"checks":ck,"coverage_gap":STATE.get("coverage_gap"),
        "heartbeat_age_s":round(age,1),"strategy":eng.STRATEGY,"storage_backend":db.backend,"durable_storage":DURABLE_STORAGE,
        "last_error":STATE.get("last_error")},status_code=200 if ok else 503)
...
async def api_audit():
    ...
    return {"strategy":eng.STRATEGY,"mode":"PAPER_STAGING","release_sha":RELEASE_SHA,
        "storage":{"backend":db.backend,"durable":DURABLE_STORAGE},"risk_controls":STATE["risk_controls"],
        "prospective":db.audit_summary(COST,START_EQUITY,now_ms),"recent_decisions":db.recent_decisions(20),"recent_trades":db.recent(20)}
