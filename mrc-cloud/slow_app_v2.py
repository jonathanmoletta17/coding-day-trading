...
def load_telemetry():
    ...
    for s in SYMBOLS:
        ...
        counts[s]=db.decision_count(s)
        decided[s]=d["decided_at"] if d else None
    return {
        "last_processed_close":last,
        "processed_close_count":counts,
        "last_decision_at":decided,
        "signals_created":db.signal_count(),
        "paper_positions_opened":db.trade_count(),
    }

def refresh_telemetry():
    STATE["telemetry"]=load_telemetry()
...
async def manage_open(client,now_ms):
    ...
    if not rows:return
    oldest=rows[0]["ot"];expected=int(t["last_check_ms"] or t["opened_ms"])
    if oldest>expected+eng.MINUTE:
        STATE["coverage_gap"]=f"{t['symbol']} 1M_COVERAGE_GAP from {iso(expected)} to {iso(oldest)}";return
...
async def loop():
    ...
    try:
        while True:
            ...
            daily_pnl=db.daily_realized_pnl(now_ms)
            daily_locked=daily_pnl<=-(START_EQUITY*DAILY_LOCK_PCT)
            ...
            for s in SYMBOLS:
                try:
                    ...
                    ctx,p=eng.evaluate(s,h1,h4,bid,ask,now_ms,db.equity(START_EQUITY),RISK_PCT,slot_open=slot,daily_locked=daily_locked)
                    action="WATERMARK_ONLY"
                    if process:
                        decided_at=iso(now_ms)
                        if p:
                            db.record_signal(p,decided_at);action=p.decision
                        else:
                            action=ctx.get("state","UNKNOWN")
                    ...
                    async with LOCK:
                        STATE["symbols"][s]={"context":ctx,"candidate":p.__dict__ if p else None,
                            "watermark":newmark if current_close else None,"processed_this_cycle":bool(process),
...
@app.get("/healthz")
async def health():
    age=time.time()-STATE.get("heartbeat",0);ck=checks();process_ok=TASK is not None and not TASK.done() and age<90
    ...
    return JSONResponse({"ok":ok,"feed_ready":bool(ck and all(ck.values())),"checks":ck,
        "coverage_gap":STATE.get("coverage_gap"),"heartbeat_age_s":round(age,1),"strategy":eng.STRATEGY,
        "storage_backend":db.backend,"durable_storage":DURABLE_STORAGE,"last_error":STATE.get("last_error")},status_code=200 if ok else 503)

@app.get("/readyz")
async def ready():
    ck=checks();ok=bool(ck and all(ck.values()) and not STATE.get("coverage_gap"))
    return JSONResponse({"ready":ok,"checks":ck,"coverage_gap":STATE.get("coverage_gap"),"strategy":eng.STRATEGY,
        "mode":"PAPER_STAGING","storage_backend":db.backend,"durable_storage":DURABLE_STORAGE,"release_sha":RELEASE_SHA},status_code=200 if ok else 503)
...
async def api_audit():
    ...
    return {
        "strategy":eng.STRATEGY,"mode":"PAPER_STAGING","release_sha":RELEASE_SHA,
        "storage":{"backend":db.backend,"durable":DURABLE_STORAGE},
        "risk_controls":STATE["risk_controls"],
        "prospective":db.audit_summary(COST,START_EQUITY,now_ms),
        "recent_decisions":db.recent_decisions(20),
        "recent_trades":db.recent(20),
    }

@app.get("/",response_class=HTMLResponse)
async def root():
    return """<html><body style='background:#071019;color:#eaf2f8;font-family:system-ui;padding:28px'>
    <h1>MRC Slow Trend — PAPER AUDIT</h1><p>4H EMA20/50 + 1H Donchian20 + ATR14 · PAPER ONLY</p>
    <p><a style='color:#70c7ff' href='/api/state'>State</a> · <a style='color:#70c7ff' href='/api/audit'>Audit</a> · <a style='color:#70c7ff' href='/readyz'>Readiness</a></p>
    <div id='x'>loading…</div><script>
    async function g(){let r=await fetch('/api/audit'),x=await r.json();document.querySelector('#x').innerHTML='<pre>'+JSON.stringify(x,null,2)+'</pre>'};g();setInterval(g,10000)
    </script></body></html>"""
