...
echo [1/4] Parando processos Python existentes na porta 18005...
for /f "tokens=5" %%a in ('netstat -ano ^| findstr :18005 ^| findstr LISTENING') do (
    echo    Matando PID: %%a
...
netstat -ano | findstr :18005 | findstr LISTENING >nul
if %errorlevel% equ 0 (
    echo    [ERRO] Porta 18005 ainda em uso
    echo    Execute este script novamente
    pause
    exit /b 1
) else (
    echo    [OK] Porta 18005 livre
)
