...
EXPECTED_RELEASE = os.getenv("MRC_EXPECTED_PAPER_RELEASE", "119a1e9a88da76ff31130804340bb0655c97d990").strip()
...
def sample_band(closed: int) -> str:
    if closed <= 0:return "N0"
    if closed < 10:return "N1_9"
    if closed < 30:return "N10_29"
    if closed < 50:return "N30_49"
    return "N50_PLUS"
...
def evaluate(ready: dict, audit: dict, costs: dict, evidence: dict, previous: dict | None = None) -> dict:
    ...
    monotonic={"decision_events_monotonic":True,"breakout_candidates_monotonic":True,"paper_trades_monotonic":True,"closed_trades_monotonic":True}
    regression_details=[]
    if previous and previous.get("paper_release")==current_release:
        pe=previous.get("economic_evidence") or {}
        pairs=(("decision_events",decisions,"decision_events_monotonic"),("breakout_candidates",candidates,"breakout_candidates_monotonic"),
               ("paper_trades_total",paper_trades,"paper_trades_monotonic"),("closed_trades",closed,"closed_trades_monotonic"))
    ...
    if closed==0:
        ...
        review_links_ok=True;review_costs_ok=True
    else:
        review_state_ok=review.get("available") is True
        review_links_ok=review.get("link_integrity_pass") is True
        review_costs_ok=all(k in (review.get("cost_scenarios") or {}) for k in ("baseline_6bps","stress_10bps","stress_15bps"))

    operational_checks={
        "paper_ready":ready.get("ready") is True,"btc_ready":(ready.get("checks") or {}).get("BTCUSDT") is True,
        "eth_ready":(ready.get("checks") or {}).get("ETHUSDT") is True,"coverage_gap_clear":ready.get("coverage_gap") is None,
        "durable_storage":storage.get("durable") is True and ready.get("durable_storage") is True,
        "release_pinned_across_endpoints":release_chain_ok,"paper_mode":audit.get("mode")=="PAPER_STAGING"==evidence.get("mode"),
        ...
        "latest_closed_trade_review_read_only":review_read_only,
    ...
    economic={
        "sample_state":sample_state,"sample_band":sample_band(closed),"sample_band_role":"descriptive_only_not_an_automatic_promotion_threshold",
        "decision_events":decisions,"breakout_candidates":candidates,"paper_trades_total":paper_trades,"closed_trades":closed,"open_position":open_position,
        "wins":p.get("wins"),"losses":p.get("losses"),"win_rate":p.get("win_rate"),"expectancy_net_R":p.get("expectancy_net_R"),
    ...
    if not all(operational_checks.values()):r0_block_reasons.insert(0,"PAPER_OPERATIONAL_GATE_NOT_CLEAN")
    fingerprint_payload={"paper_release":current_release,"operational_checks":operational_checks,"economic_evidence":economic,"regression_details":regression_details}
    fingerprint=hashlib.sha256(json.dumps(fingerprint_payload,sort_keys=True,separators=(",",":"),ensure_ascii=False,default=str).encode()).hexdigest()
    return {"status":"PASS" if all(operational_checks.values()) else "FAIL","phase":"PROSPECTIVE_EVIDENCE_COLLECTION","strategy":audit.get("strategy"),
            "paper_release":current_release,"collected_at":utc_now(),"poll_seconds":POLL_SECONDS,"snapshot_fingerprint_sha256":fingerprint,
            "operational_checks":operational_checks,"regression_details":regression_details,"economic_evidence":economic,
            "r0":{"real_money_allowed":False,"automatic_promotion_supported":False,"status":"BLOCKED","block_reasons":r0_block_reasons},
...
def compact_history_item(result: dict) -> dict:
    ...
    return {"snapshot_seq":result.get("snapshot_seq"),"collected_at":result.get("collected_at"),"status":result.get("status"),
...
def log_mode(result: dict,last_fingerprint: str|None,last_status: str|None,seq_since_log: int)->str:
    if result.get("error") or result.get("regression_details"):return "FULL"
    if last_fingerprint is None or result.get("snapshot_fingerprint_sha256")!=last_fingerprint:return "FULL"
    if last_status is None or result.get("status")!=last_status:return "FULL"
    if seq_since_log>=LOG_HEARTBEAT_POLLS:return "HEARTBEAT"
    return "NONE"
...
def emit_log(result: dict)->None:
    ...
    if mode=="NONE":return
    if mode=="FULL":print("FORWARD_PAPER_EVIDENCE_GATE="+json.dumps(result,separators=(",",":"),ensure_ascii=False),flush=True)
    else:
        e=result.get("economic_evidence") or {};compact={"snapshot_seq":result.get("snapshot_seq"),"status":result.get("status"),"paper_release":result.get("paper_release"),
            "fingerprint":result.get("snapshot_fingerprint_sha256"),"decision_events":e.get("decision_events"),"breakout_candidates":e.get("breakout_candidates"),
            "paper_trades_total":e.get("paper_trades_total"),"closed_trades":e.get("closed_trades"),"sample_band":e.get("sample_band"),"r0":(result.get("r0") or {}).get("status")}
        print("FORWARD_PAPER_EVIDENCE_HEARTBEAT="+json.dumps(compact,separators=(",",":"),ensure_ascii=False),flush=True)
    LAST_LOGGED_FINGERPRINT=result.get("snapshot_fingerprint_sha256");LAST_LOGGED_STATUS=result.get("status");LAST_LOGGED_SEQ=int(result.get("snapshot_seq") or 0)
...
def poll_loop(stop: threading.Event)->None:
    while not stop.wait(POLL_SECONDS):refresh_once()

def snapshot()->dict:
    with LOCK:return json.loads(json.dumps(RESULT))
def history()->list[dict]:
    with LOCK:return list(HISTORY)

class H(BaseHTTPRequestHandler):
    def _json(self,body_obj:object,status:int=200):
        body=json.dumps(body_obj,separators=(",",":")).encode();self.send_response(status);self.send_header("Content-Type","application/json");self.send_header("Content-Length",str(len(body)));self.end_headers();self.wfile.write(body)
    def do_GET(self):
        current=snapshot()
        if self.path=="/healthz":
            collected_at=current.get("collected_at");age=None
            if collected_at:
                try:age=time.time()-datetime.fromisoformat(collected_at).timestamp()
                except Exception:age=None
            fresh=age is not None and age<=MAX_STALE_SECONDS;ok=current.get("status")=="PASS" and fresh
            self._json({"ok":ok,"status":current.get("status"),"phase":current.get("phase"),"snapshot_seq":current.get("snapshot_seq"),
                        "snapshot_fingerprint_sha256":current.get("snapshot_fingerprint_sha256"),"collected_at":collected_at,"snapshot_age_s":round(age,1) if age is not None else None,
                        "fresh":fresh,"paper_release":current.get("paper_release"),"r0":current.get("r0")},200 if ok else 503)
        elif self.path in ("/","/snapshot"):self._json(current,200 if current.get("status")=="PASS" else 503)
        elif self.path=="/history":self._json({"persistence":"memory_only","items":history()})
        else:self._json({"error":"not_found"},404)
    def log_message(self,*args):return

def main():
    refresh_once();stop=threading.Event();worker=threading.Thread(target=poll_loop,args=(stop,),daemon=True,name="paper-evidence-poller");worker.start()
    try:ThreadingHTTPServer(("0.0.0.0",int(os.getenv("PORT","8080"))),H).serve_forever()
    finally:stop.set()
